...
const (
	...
	checkFunctionsExists    = "select * from pg_catalog.pg_proc where proname='%[1]s_check_version'"
	createVersionCheckFunc  = "create function %[1]s_check_version(current int) returns bool" +
	...
	dropServiceData = "drop table if exists %[1]s; drop function if exists %[1]s_check_version"
)
